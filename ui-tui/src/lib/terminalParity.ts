...
export async function terminalParityHints(
  ...
  if (
    ...
    hints.push({
      key: 'ide-setup',
      tone: 'info',
      message: `检测到 ${ctx.vscodeLike} 终端 · 运行 /terminal-setup 可改善 Cmd+Enter / 撤销快捷键一致性`
    })
  }
  ...
  if (ctx.isAppleTerminal) {
    hints.push({
      ...
      tone: 'warn',
      message:
        '检测到 Apple Terminal · 纯图片剪贴板可用 /paste；如果 Cmd+←/→/⌫ 被改写，可试 Ctrl+A / Ctrl+E / Ctrl+U'
    })
  }
  ...
  if (ctx.isTmux) {
    hints.push({
      ...
      tone: 'warn',
      message:
        '检测到 tmux · 剪贴板复制/粘贴会尽量使用 passthrough；allow-passthrough 可提升 OSC52 可靠性'
    })
  }
