...
export const MessageLine = memo(function MessageLine({
  ...
  // feeds Thinking + tool-call rows.  Gating on every section would let
  ...
  if (msg.role === 'tool') {
    ...
    const preview = compactPreview(stripped, maxChars) || ZH.emptyToolResult
  ...
  const content = (() => {
    ...
    if (systemIsLong) {
      const firstLine = (msg.text.split('\n')[0] ?? '').trim().slice(0, 120) || ZH.emptySystemMessage
