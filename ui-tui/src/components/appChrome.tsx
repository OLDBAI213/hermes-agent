...
const modelLabel = (model: string, effort?: string, fast?: boolean) =>
  [shortModelLabel(model), effortLabel(effort), fast ? '快速' : ''].filter(Boolean).join(' ')

const STATUS_LABELS: Record<string, string> = {
  'forging session…': '正在创建会话…',
  interrupted: '已中断',
  'interpolating…': '正在补全…',
  'queued for next turn': '已加入下一轮',
  ready: '就绪',
  resuming: '正在恢复',
  'resuming most recent…': '正在恢复最近会话…',
  'resuming…': '正在恢复…',
  'running…': '运行中…',
  'session not ready yet': '会话尚未就绪',
  'setup running…': '正在设置…',
  'setup required': '需要设置',
  'starting agent…': '正在启动 Agent…',
  'summoning hermes…': '正在唤起 Hermes…'
}
...
export function StatusRule({
  ...
  const ctxLabel = showMeter && usage.context_max
    ? `${fmtK(usage.context_used ?? 0)}/${fmtK(usage.context_max)}`
    : showMeter && usage.total > 0
      ? `${fmtK(usage.total)} tok`
      : ''

  const bar = showMeter && usage.context_max ? ctxBar(pct) : ''
    ...
    <Box height={1}>
      <Box flexShrink={1} width={leftWidth}>
        <Text color={t.color.border} wrap="truncate-end">
          ...
          {bgCount > 0 ? <Text color={t.color.muted}> │ {bgCount} 后台</Text> : null}
