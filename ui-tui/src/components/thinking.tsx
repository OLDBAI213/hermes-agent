...
function SubagentAccordion({
  ...
  const goalLabel = item.goal || `子任务 ${item.index + 1}`
  ...
  const statusLabel = statusText(String(item.status))
  ...
  if (localTokens > 0) {
    rollupBits.push(`${fmtTokens(localTokens)} tokens`)
  }
    ...
    sections.push({
      header: (
        <Chevron
          ...
          open={openKids}
          suffix={`d${item.depth + 1} · 共 ${aggregate.descendantCount} 个`}
          t={t}
          title="已启动子任务"
        />
      ),
...
export const ToolTrail = memo(function ToolTrail({
  ...
  for (const [i, line] of trail.entries()) {
    ...
    if (line.startsWith('drafting ') || line.startsWith('正在准备 ')) {
      const raw = line.startsWith('drafting ') ? line.slice(9) : line.slice(5)
      const label = toolTrailLabel(raw.replace(/…$/, '').trim())

      groups.push({
        color: t.color.text,
        content: label,
        details: [{ color: t.color.muted, content: '正在准备…', dimColor: true, key: `tr-${i}-d` }],
        key: `tr-${i}`,
        label
      })
    ...
    if (line === 'analyzing tool output…' || line === '正在分析工具输出…') {
      pushDetail({
        color: t.color.muted,
        dimColor: true,
        key: `tr-${i}`,
  ...
  const totalTokensLabel = tokenCount > 0 && toolTokenCount > 0 ? `总计 ~${fmtK(totalTokenCount)}` : null
  const delegateGroups = groups.filter(g => g.label.startsWith('Delegate Task') || g.label.startsWith('委托任务'))
