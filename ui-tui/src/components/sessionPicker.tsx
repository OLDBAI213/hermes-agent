  ...
  return (
    <Box flexDirection="column" width={width}>
        ...
        return (
          <Box key={s.id}>
            ...
            <Text
              ...
              wrap="truncate-end"
            >
              {pendingDelete ? '再次按 d 删除' : s.title || s.preview || '(未命名)'}
            </Text>
          </Box>
        )
      })}
      ...
      {offset + VISIBLE < items.length && (
        <Text color={t.color.muted}>  ↓ 还有 {items.length - offset - VISIBLE} 条</Text>
      )}
      {err && <Text color={t.color.label}>错误: {err}</Text>}
      {deleting ? (
        <OverlayHint t={t}>正在删除…</OverlayHint>
      ) : (
        <OverlayHint t={t}>↑/↓ 选择 · Enter 恢复 · 1-9 快选 · d 删除 · Esc/q 取消</OverlayHint>
      )}
    </Box>
  )
}
