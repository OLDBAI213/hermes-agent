  ...
  return (
    <Box flexDirection="column" marginBottom={1}>
      <Text color={t.color.muted}>
        时间线 · {fmtElapsedLabel(Math.max(0, totalSeconds))}
        {windowLabel}
      </Text>
...
function Detail({ id, node, t }: { id?: string; node: SubagentNode; t: Theme }) {
  ...
  // Tool rows: prefer the live stream; for archived / post-turn views
    ...
    <Box flexDirection="column">
      ...
      <Box flexDirection="column" marginTop={1}>
        ...
        <Field
          name="子树"
          t={t}
          value={`${agg.descendantCount} 个 Agent · d${agg.maxDepthFromHere} · ⚡${agg.activeCount}`}
        />
      ...
      {localTokens > 0 || localCost > 0 ? (
        <OverlaySection defaultOpen t={t} title="预算">
          ...
          {subtreeTokens > 0 ? <Field name="子树 tokens" t={t} value={`+${fmtTokens(subtreeTokens)}`} /> : null}
        </OverlaySection>
      ) : null}

      {filesRead.length > 0 || filesWritten.length > 0 ? (
        <OverlaySection count={filesRead.length + filesWritten.length} t={t} title="文件">
          ...
          {filesOverflow > 0 ? <Text color={t.color.muted}>…还有 {filesOverflow} 项</Text> : null}
        </OverlaySection>
      ) : null}

      {toolLines.length > 0 ? (
        <OverlaySection count={toolLines.length} defaultOpen t={t} title="🛠 工具调用记录">
      ...
      {outputTail.length > 0 ? (
        <OverlaySection count={outputTail.length} defaultOpen t={t} title="输出">
      ...
      {item.notes.length ? (
        <OverlaySection count={item.notes.length} t={t} title="进度">
      ...
      {item.summary ? (
        <OverlaySection defaultOpen t={t} title="总结">
...
function ListRow({
  ...
  const goal = compactPreview(node.item.goal || '子 Agent', width - 28 - node.item.depth * 2)
  const toolsCount = node.aggregate.totalTools > 0 ? ` ·${node.aggregate.totalTools}工具` : ''
          ...
          .map(s => {
            ...
            return (
              <Text color={t.color.muted} key={s.id} wrap="truncate-end">
                <Text color={color}>{glyph}</Text> {s.goal || '子 Agent'}
              </Text>
            )
          })}
      </Box>
    ...
    <Box flexDirection="column" flexGrow={1} paddingX={1} paddingY={1}>
      ...
      <Box flexDirection="column" marginTop={1}>
        ...
        <Text color={t.color.text}>
          {diffMetricLine('Agent', aTotals.descendantCount, bTotals.descendantCount, round)}
        </Text>
        ...
        <Text color={t.color.text}>{diffMetricLine('费用', aTotals.costUsd, bTotals.costUsd, dollars)}</Text>
      </Box>
    </Box>
  )
}
...
export function AgentsOverlay({ gw, initialHistoryIndex = 0, onClose, t }: AgentsOverlayProps) {
    ...
    guardLive(() => {
      interrupt(id)
        .then(raw => {
          ...
          setFlash(r?.found ? `正在终止 ${id}` : `未找到: ${id}`)
        })
        .catch(() => setFlash(`终止失败: ${id}`))
    })
  ...
  const stepHistory = (delta: -1 | 1) =>
    setHistoryIndex(idx => {
      ...
      if (next !== idx) {
        setCursor(0)
        setFlash(next === 0 ? '实时回合' : `回放 · ${next}/${history.length}`)
      }
  ...
  const mix = Object.entries(
    subagents.reduce<Record<string, number>>((acc, it) => {
      const key = it.model ? it.model.split('/').pop()! : '继承'
      acc[key] = (acc[key] ?? 0) + 1
    ...
    <Box alignItems="stretch" flexDirection="column" flexGrow={1} paddingX={1} paddingY={1}>
      ...
      {rows.length === 0 ? (
        <Box flexDirection="column" flexGrow={1}>
          <Text color={t.color.muted}>本轮没有子 Agent。触发 delegate_task 后这里会显示调用树。</Text>
        </Box>
      ) : mode === 'list' ? (
      ...
      <Box flexDirection="column" marginTop={1}>
        ...
        {mode === 'list' ? (
          <Text color={t.color.muted}>
            ↑↓/jk 移动 · g/G 顶部/底部 · Enter/→ 打开详情{controlsHint} · s 排序:{SORT_LABEL[sort]} · f 过滤:
            {FILTER_LABEL[filter]}
