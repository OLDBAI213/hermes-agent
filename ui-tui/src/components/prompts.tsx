...
const LABELS = { always: '始终允许', deny: '拒绝', once: '允许一次', session: '本会话允许' } as const
...
export function ApprovalPrompt({ onChoice, req, t }: ApprovalPromptProps) {
  ...
  return (
    <Box borderColor={t.color.warn} borderStyle="double" flexDirection="column" paddingX={1}>
      ...
      <Text color={t.color.muted}>↑/↓ 选择 · Enter 确认 · 1-4 快选 · Esc/Ctrl+C 拒绝</Text>
    </Box>
  )
}
...
export function ClarifyPrompt({ cols = 80, onAnswer, onCancel, req, t }: ClarifyPromptProps) {
  ...
  const heading = (
    <Text bold>
      <Text color={t.color.accent}>询问</Text>
  ...
  if (typing || !choices.length) {
    return (
      <Box flexDirection="column">
        ...
        <Text color={t.color.muted}>
          Enter 发送 · Esc {choices.length ? '返回' : '取消'} ·{' '}
          {isMac ? 'Cmd+C 复制 · Cmd+V 粘贴 · Ctrl+C 取消' : 'Ctrl+C 取消'}
        </Text>
      </Box>
    )
  }
  ...
  return (
    <Box flexDirection="column">
      ...
      <Text color={t.color.muted}>↑/↓ 选择 · Enter 确认 · 1-{choices.length} 快选 · Esc/Ctrl+C 取消</Text>
    </Box>
  )
}
...
export function ConfirmPrompt({ onCancel, onConfirm, req, t }: ConfirmPromptProps) {
  ...
  const rows = [
    { color: t.color.text, label: req.cancelLabel ?? '否' },
    { color: req.danger ? t.color.error : t.color.text, label: req.confirmLabel ?? '是' }
  ]
  ...
  return (
    <Box borderColor={accent} borderStyle="double" flexDirection="column" paddingX={1}>
      ...
      <Text color={t.color.muted}>↑/↓ 选择 · Enter 确认 · Y/N 快选 · Esc 取消</Text>
    </Box>
  )
}
