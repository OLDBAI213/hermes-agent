...
export function SkillsHub({ gw, onClose, t }: SkillsHubProps) {
  ...
  if (loading) {
    return <Text color={t.color.muted}>正在加载技能…</Text>
  }
  ...
  if (!cats.length) {
    return (
      <Box flexDirection="column" width={width}>
        <Text color={t.color.muted}>没有可用技能</Text>
        <OverlayHint t={t}>Esc/q 取消</OverlayHint>
      </Box>
    )
  }
  ...
  if (stage === 'category') {
    const rows = cats.map(c => `${c} · ${skillsByCat[c]?.length ?? 0} 个技能`)
    ...
    return (
      <Box flexDirection="column" width={width}>
        ...
        <Text color={t.color.muted}>选择分类</Text>
        {offset > 0 && <Text color={t.color.muted}> ↑ 还有 {offset} 项</Text>}
        ...
        {offset + VISIBLE < rows.length && (
          <Text color={t.color.muted}> ↓ 还有 {rows.length - offset - VISIBLE} 项</Text>
        )}
        <OverlayHint t={t}>↑/↓ 选择 · Enter 打开 · 1-9,0 快选 · Esc/q 取消</OverlayHint>
      </Box>
    )
  }
  ...
  if (stage === 'skill') {
    ...
    return (
      <Box flexDirection="column" width={width}>
        ...
        <Text color={t.color.muted}>{skills.length} 个技能</Text>
        {!skills.length ? <Text color={t.color.muted}>这个分类下没有技能</Text> : null}
        {offset > 0 && <Text color={t.color.muted}> ↑ 还有 {offset} 项</Text>}
        ...
        {offset + VISIBLE < skills.length && (
          <Text color={t.color.muted}> ↓ 还有 {skills.length - offset - VISIBLE} 项</Text>
        )}
        <OverlayHint t={t}>
          {skills.length ? '↑/↓ 选择 · Enter 打开 · 1-9,0 快选 · Esc 返回 · q 关闭' : 'Esc 返回 · q 关闭'}
        </OverlayHint>
      </Box>
    )
  }
  ...
  return (
    <Box flexDirection="column" width={width}>
      ...
      {info?.path ? <Text color={t.color.muted}>路径: {info.path}</Text> : null}
      {!info && !err ? <Text color={t.color.muted}>正在加载…</Text> : null}
      {err ? <Text color={t.color.label}>错误: {err}</Text> : null}
      {installing ? <Text color={t.color.accent}>正在安装…</Text> : null}

      <OverlayHint t={t}>i 重新检查 · x 重新安装 · Enter/Esc 返回 · q 关闭</OverlayHint>
    </Box>
  )
}
