...
import { isTuiModuleEnabled } from '../domain/tuiModules.js'
...
export const StreamingAssistant = memo(function StreamingAssistant({
  ...
  const showActivityScan = isTuiModuleEnabled(ui.tuiModules, 'activity_scan', cols)
  const showStreamingArea = Boolean(streaming)

  if (!progress.showProgressArea && !showStreamingArea && (!showActivityScan || !activeTools.length)) {
    return null
  }
  ...
  return (
    <>
      ...
      {showActivityScan && !!activeTools.length && (
        <MessageLine
          cols={cols}
          compact={compact}
          detailsMode={detailsMode}
      ...
      {showStreamingArea && (
        <MessageLine
          ...
          msg={{
            role: 'assistant',
            text: streaming,
            ...(showActivityScan && streamPendingTools.length && { tools: streamPendingTools })
          }}
          sections={sections}
          t={ui.theme}
        />
      ...
      {showActivityScan && !showStreamingArea && !!streamPendingTools.length && (
        <MessageLine
          cols={cols}
          compact={compact}
          detailsMode={detailsMode}
