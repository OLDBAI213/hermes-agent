...
export function PromptZone({
  ...
  if (overlay.sudo) {
    return (
      <Box flexDirection="column" flexShrink={0} paddingX={1} paddingY={1}>
        <MaskedPrompt cols={cols} icon="🔐" label="需要 sudo 密码" onSubmit={onSudoSubmit} t={theme} />
      </Box>
    )
  }
            ...
            <Box marginTop={1}>
              <OverlayHint t={theme}>
                {overlay.pager.offset + pagerPageSize < overlay.pager.lines.length
                  ? `↑↓/jk 逐行 · Enter/Space/PgDn 翻页 · b/PgUp 返回 · g/G 首尾 · Esc/q 关闭 (${Math.min(overlay.pager.offset + pagerPageSize, overlay.pager.lines.length)}/${overlay.pager.lines.length})`
                  : `已到底 · ↑↓/jk · b/PgUp 返回 · g 顶部 · Esc/q 关闭（共 ${overlay.pager.lines.length} 行）`}
              </OverlayHint>
            </Box>
          </Box>
        </FloatBox>
