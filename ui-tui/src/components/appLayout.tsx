        ...
        <Box flexDirection="column" paddingX={1}>
          ...
          {transcript.virtualRows.slice(transcript.virtualHistory.start, transcript.virtualHistory.end).map(row => (
            <Box flexDirection="column" key={row.key} ref={transcript.virtualHistory.measureRef(row.key)}>
              ...
              {row.index === lastUserIdx && isTuiModuleEnabled(ui.tuiModules, 'task_panel', composer.cols) && (
                <LiveTodoPanel />
              )}
            </Box>
          ))}
    ...
    >
      ...
      {ui.bgTasks.size > 0 && (
        <Text color={ui.theme.color.muted}>
          {ZH.backgroundTasks(ui.bgTasks.size)}
        </Text>
      )}
            ...
            >
              ...
              <Box flexGrow={0} flexShrink={0} height={inputHeight} width={inputColumns}>
                ...
                <TextInput
                  ...
                  placeholder={composer.empty ? PLACEHOLDER : ui.busy ? 'Ctrl+C 中断…' : ''}
  ...
  return (
    <Box marginTop={at === 'top' ? 1 : 0}>
      <StatusRule
        ...
        showMeter={isTuiModuleEnabled(ui.tuiModules, 'status_meter', composer.cols)}
        status={ui.status}
