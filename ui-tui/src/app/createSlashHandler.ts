...
export function createSlashHandler(ctx: SlashHandlerContext): (cmd: string) => boolean {
  ...
  const handler = (cmd: string): boolean => {
    ...
    const guardedErr = (e: unknown) => {
      if (!stale()) {
        sys(ZH.error(rpcErrorMessage(e)))
      }
    }
    ...
    gw.request<SlashExecResponse>('slash.exec', { command: cmd.slice(1), session_id: sid })
      .then(r => {
        ...
        const text = r?.warning ? `${ZH.warning(r.warning)}\n${body}` : body
        const long = text.length > 180 || text.split('\n').filter(Boolean).length > 2

        long ? page(text, parsed.name) : sys(text)
      })
      .catch(() => {
        gw.request('command.dispatch', { arg: parsed.arg, name: parsed.name, session_id: sid })
          .then((raw: unknown) => {
            ...
            if (!d) {
              return sys(ZH.invalidResponse('command.dispatch'))
            }

            if (d.type === 'exec' || d.type === 'plugin') {
              return sys(d.output || ZH.noOutput)
            }
            ...
            if (d.type === 'skill') {
              sys(ZH.loadingSkill(d.name))

              return d.message?.trim() ? send(d.message) : sys(ZH.skillPayloadMissing(parsed.name))
            }
            ...
            if (d.type === 'send') {
              ...
              return d.message?.trim() ? send(d.message) : sys(ZH.emptyMessage(parsed.name))
            }
          })
          .catch(guardedErr)
      })
