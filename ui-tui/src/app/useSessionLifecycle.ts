...
export function useSessionLifecycle(opts: UseSessionLifecycleOptions) {
  ...
  const newSession = useCallback(
    async (msg?: string, title?: string) => {
      ...
      if (info?.credential_warning) {
        sys(ZH.warning(info.credential_warning))
      }

      if (info?.config_warning) {
        sys(ZH.warning(info.config_warning))
      }
      ...
      if (requestedTitle) {
        rpc<SessionTitleResponse>('session.title', {
          ...
          .then(result => {
            ...
            const suffix = result.pending ? ZH.queuedWhileSessionInitializes : ''
            sys(ZH.sessionTitleSet(nextTitle, suffix))
          })
          .catch((err: unknown) => {
            ...
            sys(ZH.setSessionTitleFailed(message))
          })
      }
    },
        ...
        closeSession(getUiState().sid === id ? null : getUiState().sid).then(() =>
          gw
            ...
            .then(raw => {
              ...
              if (!r) {
                sys(ZH.invalidResponse('session.resume'))
              ...
              setHistoryItems(resumed.length ? resumed : r.info ? [introMsg(r.info)] : [])
  ...
  const guardBusySessionSwitch = useCallback(
    (what = 'switch sessions') => {
      ...
      sys(ZH.interruptCurrentTurnFirst)
