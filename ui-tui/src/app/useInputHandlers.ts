...
export function useInputHandlers(ctx: InputHandlerContext): InputHandlerResult {
  ...
  const cancelOverlayFromCtrlC = () => {
    ...
    if (overlay.sudo) {
      return gateway
        ...
        .then(r => r && (patchOverlayState({ sudo: null }), actions.sys(ZH.sudoCancelled)))
    }

    if (overlay.secret) {
      return gateway
        .rpc<SecretRespondResponse>('secret.respond', { request_id: overlay.secret.requestId, value: '' })
        .then(r => r && (patchOverlayState({ secret: null }), actions.sys(ZH.promptCancelled)))
    }
  ...
  const voiceRecordToggle = () => {
    if (!voice.enabled) {
      return actions.sys(ZH.voiceModeOff)
    }
    ...
    gateway
      ...
      .catch((e: Error) => {
        ...
        actions.sys(ZH.voiceError(e.message))
      })
  }
  ...
  useInput((ch, key) => {
    ...
    if (ch.toLowerCase() === 'g' && (isAction(key, ch, 'g') || key.meta)) {
      return void cActions.openEditor().catch((err: unknown) => {
        actions.sys(ZH.failedToOpenEditor(err instanceof Error ? err.message : undefined))
      })
    }
