  ...
  return (ev: GatewayEvent) => {
    ...
    switch (ev.type) {
      ...
      case 'gateway.start_timeout': {
        ...
        setStatus('网关启动超时')
        turnController.pushActivity(`网关启动超时${trace} · 可用 /logs 查看`, 'error')
      ...
      case 'gateway.protocol_error':
        ...
        if (!turnController.protocolWarned) {
          ...
          turnController.pushActivity('检测到协议噪声 · 可用 /logs 查看', 'info')
        }

        if (ev.payload?.preview) {
          turnController.pushActivity(`协议噪声: ${String(ev.payload.preview).slice(0, 120)}`, 'info')
        }
