...
export const coreCommands: SlashCommand[] = [
  {
    ...
    run: (_arg, ctx) => {
      ...
      sections.push(
        {
          rows: [
            ['/details [hidden|collapsed|expanded|cycle]', '设置全局 Agent 细节显示模式'],
            [
              '/details <section> [hidden|collapsed|expanded|reset]',
              '单独设置一个分区 (thinking/tools/subagents/activity)'
            ],
            ['/fortune [random|daily]', '显示随机或每日本地签文']
          ],
          title: 'TUI'
        },
...

  {
    ...
    run: (arg, ctx) => {
      ...
      if (next === null) {
        return ctx.transcript.sys('用法: /mouse [on|off|toggle]')
      }

      patchUiState({ mouseTracking: next })
      ctx.gateway.rpc<ConfigSetResponse>('config.set', { key: 'mouse', value: next ? 'on' : 'off' }).catch(() => {})

      queueMicrotask(() => ctx.transcript.sys(`鼠标跟踪已${next ? '开启' : '关闭'}`))
    }
  },

  {
    ...
    run: (arg, ctx, cmd) => {
      ...
      const commit = () => {
        ...
        ctx.session.newSession(isNew ? '已开始新会话' : undefined, requestedTitle || undefined)
      }
      ...
      patchOverlayState({
        confirm: {
          cancelLabel: '否，继续当前会话',
          confirmLabel: isNew ? '是，开始新会话' : '是，清空当前会话',
          danger: true,
          detail: '这会结束当前对话并清空界面记录。',
          onConfirm: commit,
        ...
        ctx.gateway
          ...
          .then(
            ctx.guarded<SessionTitleResponse>(r => {
              ...
              ctx.transcript.sys(current ? `标题: ${current}` : '未设置标题')
            })
          )
          .catch(ctx.guardedErr)
...

  {
    ...
    run: (arg, ctx) => {
      ...
      queueMicrotask(() => ctx.transcript.sys(`紧凑显示已${next ? '开启' : '关闭'}`))
    }
  },

  {
    aliases: ['detail'],
    help: '控制 Agent 细节显示（全局或分区）',
    name: 'details',
    run: (arg, ctx) => {
      ...
      if (!arg) {
        gateway
          ...
          .then(r => {
            ...
            transcript.sys(`细节显示: ${mode}${overrides ? `  (${overrides})` : ''}`)
          })
          .catch(() => !ctx.stale() && transcript.sys(`细节显示: ${ui.detailsMode}`))
      ...
      if (second && isSectionName(first)) {
        ...
        gateway
          ...
          .catch(() => {})
        transcript.sys(`细节显示 ${first}: ${mode ?? 'reset'}`)
    ...
    run: async (arg, ctx) => {
      ...
      if (!arg && ctx.composer.hasSelection) {
        ...
        if (text) {
          ...
          return sys('剪贴板复制失败，可设置 HERMES_TUI_FORCE_OSC52=1 强制使用 OSC52')
        }
      }

      if (arg && Number.isNaN(parseInt(arg, 10))) {
        return sys('用法: /copy [编号]')
      }
...

  {
    ...
    run: (arg, ctx) => {
      ...
      void runner
        ...
        .catch(error => {
          if (!ctx.stale()) {
            ctx.transcript.sys(`终端快捷键配置失败: ${String(error)}`)
          }
        })
    }
  },

  {
    ...
    run: (arg, ctx) => {
      ...
      text ? ctx.transcript.page(text, '日志') : ctx.transcript.sys('没有网关日志')
    }
  },

  {
    ...
    run: (arg, ctx) => {
      ...
      const lines = items.map((m, i) => {
        const tag = m.role === 'user' ? `用户 #${i + 1}` : `Hermes #${i + 1}`
...

  {
    ...
    run: (arg, ctx) => {
      if (!arg) {
        return ctx.transcript.sys(`${ctx.composer.queueRef.current.length} 条消息在队列中`)
      }

      ctx.composer.enqueue(arg)
      ctx.transcript.sys(ZH.queued(arg))
    }
  },

  {
    ...
    run: (arg, ctx) => {
      ...
      if (!payload) {
        return ctx.transcript.sys('用法: /steer <提示词>')
      }
      ...
      if (!ctx.ui.busy || !ctx.sid) {
        ...
        ctx.transcript.sys(
          `当前没有运行中的回合，已加入下一轮队列: "${payload.slice(0, 50)}${payload.length > 50 ? '…' : ''}"`
        )
      ...
      ctx.gateway
        ...
        .then(
          ctx.guarded<SessionSteerResponse>(r => {
            if (r?.status === 'queued') {
              ctx.transcript.sys(
                `插入消息已排队，将在下一次工具调用后送达: "${payload.slice(0, 50)}${payload.length > 50 ? '…' : ''}"`
              )
            } else {
