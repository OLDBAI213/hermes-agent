...
export function useMainApp(gw: GatewayClient) {
    ...
    (quiet = false) =>
      rpc<ClipboardPasteResponse>('clipboard.paste', { session_id: getUiState().sid }).then(r => {
        ...
        if (r.attached) {
          ...
          return sys(`${ZH.attachedImage(`#${r.count}`)}${meta ? ` · ${meta}` : ''}`)
        }

        if (!quiet) {
          sys(r.message || ZH.noClipboardImage)
        }
      }),
    [rpc, sys]
  )
  ...
  useEffect(() => {
    ...
    const exitHandler = () => {
      ...
      turnController.pushActivity('网关已退出 · 可用 /logs 查看', 'error')
      sys('错误: 网关已退出')
    }
