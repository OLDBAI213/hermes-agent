...
describe('createSlashHandler', () => {
  ...
  it('routes /status to live session.status instead of slash worker', async () => {
    ...
    expect(ctx.gateway.gw.request).not.toHaveBeenCalled()
    await vi.waitFor(() => {
      expect(ctx.transcript.page).toHaveBeenCalledWith('Hermes TUI Status', '状态')
    })
  })
  ...
  it('passes /new <title> through to the session lifecycle', () => {
    ...
    expect(ctx.session.newSession).toHaveBeenCalledWith('已开始新会话', 'sprint planning')
    expect(ctx.gateway.rpc).not.toHaveBeenCalled()
  })
  ...
  it('/voice on renders the configured binding for the start/stop hint', async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(ctx.transcript.sys).toHaveBeenCalledWith('  Alt+R 开始/停止录音')
    })
  ...
  it('cycles details mode and persists it', async () => {
    ...
    expect(ctx.transcript.sys).toHaveBeenCalledWith('细节显示: expanded')
  })
  ...
  it('sets a per-section override and persists it under details_mode.<section>', () => {
    ...
    expect(ctx.transcript.sys).toHaveBeenCalledWith('细节显示 activity: hidden')
  })
  ...
  it('clears a per-section override on /details <section> reset', () => {
    ...
    expect(ctx.transcript.sys).toHaveBeenCalledWith('细节显示 tools: reset')
  })
  ...
  it('renders browser connect progress messages from the gateway', async () => {
    ...
    expect(ctx.transcript.sys).toHaveBeenCalledWith('正在检查 Chromium 系浏览器远程调试地址: http://127.0.0.1:9222...')
  ...
  it('/history pages the current TUI transcript (user + assistant)', () => {
    ...
    expect(body).toContain('[用户 #1]')
    expect(body).toContain('hello')
    expect(body).toContain('[Hermes #2]')
    expect(body).toContain('hi there')
    expect(body).toContain('[用户 #3]')
  ...
  it('/save forwards to session.save RPC and reports the returned file', async () => {
    ...
    await vi.waitFor(() => {
      expect(ctx.transcript.sys).toHaveBeenCalledWith('对话已保存到: /tmp/hermes_conversation_test.json')
    })
  })
