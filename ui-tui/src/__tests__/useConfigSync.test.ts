...
describe('applyDisplay → tui_modules', () => {
  beforeEach(() => {
    resetUiState()
  })

  it('threads boolean and object module config into $uiState', () => {
    const setBell = vi.fn()
    ...
    applyDisplay(
      {
        config: {
          display: {
            tui_modules: {
              device_rail: { enabled: false, min_cols: 140, position: 'side' },
              task_panel: false
            }
          }
        }
    ...
    const modules = $uiState.get().tuiModules
    expect(modules.task_panel.enabled).toBe(false)
    expect(modules.device_rail).toEqual({ enabled: false, minCols: 140, position: 'side' })
  })

  it('keeps documented module defaults when config is missing', () => {
    const setBell = vi.fn()
