...
def _safe_home_path() -> Path:
    """Return a best-effort user home path even in stripped test envs."""
...
def get_hermes_home() -> Path:
    ...
    if not _profile_fallback_warned:
        try:
            ...
            active_path = (_safe_home_path() / ".hermes" / "active_profile")
...
def get_default_hermes_root() -> Path:
    ...
    """
    native_home = _safe_home_path() / ".hermes"
...
def display_hermes_home() -> str:
    ...
    home = get_hermes_home()
    try:
        relative = home.resolve().relative_to((_safe_home_path() / ".hermes").resolve())
        suffix = str(relative).replace("\\", "/")
        return "~/.hermes" if suffix == "." else "~/.hermes/" + suffix
    except ValueError:
        return str(home)
