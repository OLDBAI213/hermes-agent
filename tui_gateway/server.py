...
def _coerce_tui_module_enabled(raw) -> bool | None:
    ...
    if isinstance(raw, str):
        ...
        if value in _TUI_MODULE_VALUE_ON:
            return True
        if value in _TUI_MODULE_VALUE_OFF:
            return False
    return None


def _tui_module_value_label(raw) -> str:
    enabled = raw.get("enabled", True) if isinstance(raw, dict) else raw
    return "on" if _coerce_tui_module_enabled(enabled) is not False else "off"


def _set_tui_module(module_id: str, value) -> dict:
    enabled = _coerce_tui_module_enabled(value)
    if not module_id:
        raise ValueError("tui module id required")
    if enabled is None:
        raise ValueError(f"unknown tui module value: {value}")

    cfg = _load_cfg()
    display = cfg.get("display") if isinstance(cfg.get("display"), dict) else {}
    modules = (
        display.get("tui_modules")
        if isinstance(display.get("tui_modules"), dict)
        else {}
    )
    ...
    display["tui_modules"] = modules
    cfg["display"] = display
    _save_cfg(cfg)
    return {"key": f"tui_modules.{module_id}", "value": "on" if enabled else "off"}
...
def _sync_agent_system_prompt_from_config(agent) -> None:
    if not agent:
        return
    cfg = _load_cfg()
    cfg_prompt = str(((cfg.get("agent") or {}).get("system_prompt", "")) or "").strip()
    display = cfg.get("display") or {}
    language = str(display.get("language", "") or display.get("gateway_locale", "") or "").strip().lower()
    locale_prompt = ""
    if language.startswith("zh"):
        locale_prompt = (
            "语言规则：用户界面语言是简体中文。除命令、路径、代码、API 名称、日志原文、专有名词和用户要求保留的原文外，"
            "所有可见给用户的内容都必须使用简体中文。"
    ...
    if next_prompt == current:
        return
    agent.ephemeral_system_prompt = next_prompt or None
...
def _(rid, params: dict) -> dict:
    ...
    if session.get("attached_images"):
        return _ok(
            ...
            {
                "status": "rejected",
                "reason": "attached_images_require_full_turn",
                "text": text,
            },
        )
    agent = session.get("agent")
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        ...
        try:
            ...
            _sync_agent_system_prompt_from_config(agent)
...
def _(rid, params: dict) -> dict:
    ...
    if key.startswith("tui_modules."):
        module_id = key.split(".", 1)[1].strip()
        try:
            return _ok(rid, _set_tui_module(module_id, value))
        except ValueError as e:
...
def _(rid, params: dict) -> dict:
    ...
    if key.startswith("tui_modules."):
        ...
        modules = (
            display.get("tui_modules")
            if isinstance(display, dict) and isinstance(display.get("tui_modules"), dict)
            else {}
        )
        return _ok(
            rid,
            {
                "value": _tui_module_value_label(
                    modules.get(module_id, _TUI_MODULE_DEFAULTS.get(module_id, False))
                )
            },
        )
    if key == "mouse":
