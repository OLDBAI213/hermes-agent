...
def build_native_content_parts(
    ...
    Native mode must not expose local cache paths in the user-visible text.
    The model already receives the pixels via the ``image_url`` part; adding
    local path hints makes tool-capable models try ``read_file`` /
    ``vision_analyze`` instead of answering from the attached image. Paths
    remain internal gateway state only.
    ...
    couldn't be read from disk and are not advertised to the model.
    """
    skipped: List[str] = []
    ...
    # If at least one image attached, build a single text part with the user's
    # caption (or a neutral default). Do not leak local cache paths into the
    # prompt; they are implementation details and can misroute tool use.
    if attached_paths:
