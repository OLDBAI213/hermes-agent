...
class TestModelSupportsVision:
    ...
    def test_forced_native_image_mode_overrides_unknown_capability_metadata(self):
        agent = _make_agent()
        agent.provider = "xiaomi"
        agent.model = "brand-new-vision-model"
        with patch(
    ...
    def test_forced_native_image_mode_keeps_image_parts_on_chat_path(self):
        agent = _make_agent()
        agent.provider = "xiaomi"
        ...
        with patch(
            "hermes_cli.config.load_config",
            return_value={"agent": {"image_input_mode": "native"}},
        ), patch("agent.models_dev.get_model_capabilities", return_value=None), patch.object(
            agent,
            "_describe_image_for_anthropic_fallback",
    ...
    def test_known_text_only_model_is_not_forced_to_native(self):
        agent = _make_agent()
        agent.provider = "xiaomi"
        ...
        with patch(
            "hermes_cli.config.load_config",
            return_value={"agent": {"image_input_mode": "native"}},
        ), patch("agent.models_dev.get_model_capabilities", return_value=fake_caps), patch.object(
            agent,
            "_describe_image_for_anthropic_fallback",
            return_value="[Image description: fallback]",
        ):
            out = agent._prepare_messages_for_non_vision_model([IMG_PARTS_USER_MSG])
        assert isinstance(out[0]["content"], str)
        assert "image_url" not in out[0]["content"]

    def test_vision_capable_model_keeps_image_parts_with_native_mode(self):
        agent = _make_agent()
        agent.provider = "xiaomi"
        agent.model = "mimo-v2.5"
        fake_caps = MagicMock()
    ...
    def test_forced_text_image_mode_still_uses_text_fallback(self):
        agent = _make_agent()
        agent.provider = "anthropic"
        agent.model = "claude-sonnet-4"
        with patch(
            "hermes_cli.config.load_config",
            return_value={"agent": {"image_input_mode": "text"}},
        ), patch.object(
            agent,
            "_describe_image_for_anthropic_fallback",
            return_value="[Image description: fallback]",
        ...
        content = out[0]["content"]
        assert isinstance(content, str)
        assert "[Image description: fallback]" in content
        assert "image_url" not in content
