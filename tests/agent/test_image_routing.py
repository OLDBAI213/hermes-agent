...
class TestDecideImageInputMode:
    def test_explicit_native_uses_native_when_capability_is_unknown(self):
    ...
    def test_explicit_native_does_not_override_known_text_only_model(self):
        cfg = {"agent": {"image_input_mode": "native"}}
        with patch("agent.image_routing._lookup_supports_vision", return_value=False):
            assert decide_image_input_mode("xiaomi", "mimo-v2.5-pro", cfg) == "text"
...
class TestBuildNativeContentParts:
    def test_text_then_image(self, tmp_path: Path):
        ...
        # User caption is preserved, but local cache paths are not exposed.
        # Native image pixels are carried by the image_url content part.
    ...
    def test_missing_file_is_skipped(self, tmp_path: Path):
        ...
        # Skipped paths are not advertised to the model.
        assert parts == [{"type": "text", "text": "hi"}]

    def test_native_prompt_does_not_expose_local_path(self, tmp_path: Path):
        """Native image turns must not tell the model to inspect a local path."""
        ...
        assert "vision_analyze" not in text_part["text"]

    def test_missing_image_path_is_not_leaked_when_other_images_attach(self, tmp_path: Path):
        """Neither attached nor skipped local paths should be prompt text."""
        ...
        text_part = next(p for p in parts if p.get("type") == "text")
        assert "[Image attached at:" not in text_part["text"]
