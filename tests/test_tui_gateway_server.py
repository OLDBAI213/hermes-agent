...
def test_resolve_skin_includes_tui_skin_payload(tmp_path, monkeypatch):
    skins_dir = tmp_path / "skins"
    skins_dir.mkdir()
    (skins_dir / "phosphor-test.yaml").write_text(
        """
name: phosphor-test
colors:
  status_bar_bg: "#001100"
spinner:
  thinking_faces:
    - "[SCAN]"
visuals:
...
""",
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"display": {"skin": "phosphor-test"}})

    payload = server.resolve_skin()

    assert payload["name"] == "phosphor-test"
    assert payload["spinner"]["thinking_faces"] == ["[SCAN]"]
    assert payload["visuals"]["character_frames"] == ["FRAME_A", "FRAME_B"]
    assert payload["banner_logo"] == "LOGO"
    assert payload["banner_hero"] == "HERO"
    assert payload["tool_prefix"] == "|"
...
def test_config_set_tui_module_preserves_object_shape(tmp_path, monkeypatch):
    import yaml
    ...
    resp = server.handle_request(
        {
            "id": "1",
            "method": "config.set",
            "params": {"key": "tui_modules.device_rail", "value": "off"},
        }
    )

    assert resp["result"] == {"key": "tui_modules.device_rail", "value": "off"}
    saved = yaml.safe_load(cfg_path.read_text())
    assert saved["display"]["tui_modules"]["device_rail"] == {
        "enabled": False,
        "min_cols": 140,
        "position": "side",
    }
...
def test_config_set_tui_module_writes_boolean_shape(tmp_path, monkeypatch):
    import yaml

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.safe_dump({"display": {"tui_modules": {}}}))
    monkeypatch.setattr(server, "_hermes_home", tmp_path)
    ...
    resp = server.handle_request(
        {
            "id": "1",
            "method": "config.set",
            "params": {"key": "tui_modules.task_panel", "value": "on"},
        }
    )

    assert resp["result"] == {"key": "tui_modules.task_panel", "value": "on"}
    saved = yaml.safe_load(cfg_path.read_text())
    assert saved["display"]["tui_modules"]["task_panel"] is True
...
def test_config_get_tui_module_reads_boolean_and_object(monkeypatch):
    ...
    device = server.handle_request(
        {"id": "1", "method": "config.get", "params": {"key": "tui_modules.device_rail"}}
    )
    task = server.handle_request(
        {"id": "2", "method": "config.get", "params": {"key": "tui_modules.task_panel"}}
    )

    assert device["result"]["value"] == "off"
...
def test_session_steer_rejects_when_images_are_attached():
    """Attached images require a full prompt.submit turn so pixels are not
    lost through text-only steer.
    """
    calls = {}
    ...
    server._sessions["sid"] = _session(
        agent=_Agent(),
        attached_images=["E:/AI/hermes/images/photo.png"],
    )
    try:
        resp = server.handle_request(
            {
                "id": "1",
                "method": "session.steer",
                "params": {"session_id": "sid", "text": "look at this"},
            }
        )
    finally:
        server._sessions.pop("sid", None)
    ...
    assert resp["result"]["reason"] == "attached_images_require_full_turn"
