...
async def test_steer_calls_agent_steer_and_does_not_interrupt():
    ...
    assert "插入" in result or "steer" in result.lower() or "queued" in result.lower()
...
@pytest.mark.asyncio
async def test_steer_with_media_queues_full_event_instead_of_text_steer():
    runner, adapter = _make_runner(_session_entry())
    sk = build_session_key(_make_source())
    ...
    result = await runner._handle_message(
        _make_event(
            ...
            media_urls=["E:/AI/hermes/image_cache/steer.jpg"],
            media_types=["image/jpeg"],
        )
    )

    assert result is not None
    assert "图片/附件" in result
    running_agent.steer.assert_not_called()
    queued = adapter._pending_messages[sk]
    assert queued.text == "看这张图"
    assert queued.media_urls == ["E:/AI/hermes/image_cache/steer.jpg"]
    assert queued.media_types == ["image/jpeg"]


@pytest.mark.asyncio
async def test_queue_command_with_media_preserves_attachment_metadata():
    runner, adapter = _make_runner(_session_entry())
    ...
    result = await runner._handle_message(
        _make_event(
            "/queue 稍后看这张图",
            message_type=MessageType.PHOTO,
            media_urls=["E:/AI/hermes/image_cache/queue.jpg"],
            media_types=["image/jpeg"],
        )
    )
    ...
    assert "队列" in result
    queued = adapter._pending_messages[sk]
    assert queued.text == "稍后看这张图"
    assert queued.message_type == MessageType.PHOTO
    assert queued.media_urls == ["E:/AI/hermes/image_cache/queue.jpg"]
    assert queued.media_types == ["image/jpeg"]
...
async def test_steer_without_payload_returns_usage():
    ...
    assert result is not None
    assert "用法" in result or "Usage" in result or "usage" in result
    running_agent.steer.assert_not_called()
    running_agent.interrupt.assert_not_called()
...
async def test_steer_with_pending_sentinel_falls_back_to_queue():
    ...
    assert result is not None
    assert "队列" in result or "queued" in result.lower() or "starting" in result.lower()
...
async def test_steer_agent_without_steer_method_falls_back():
    ...
    assert "队列" in result or "queued" in result.lower()
    assert sk in adapter._pending_messages
...
async def test_steer_rejected_payload_returns_rejection_message():
    ...
    assert result is not None
    assert "拒绝" in result or "rejected" in result.lower() or "empty" in result.lower()
