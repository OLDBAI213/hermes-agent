...
def test_resolve_platform_delivery_and_style():
    user = {
        "display": {
            "runtime_footer": {"enabled": True},
            "platforms": {
    ...
    assert cfg["delivery"] == "status_card"
    assert cfg["fields"] == ["model", "provider", "context"]
    assert cfg["style"] == "zh_detailed"
...
def test_build_footer_returns_delivery_metadata():
    footer = build_footer(
        user_config={
            "display": {
                "platforms": {
    ...
    assert footer["delivery"] == "status_card"
    assert footer["line"] == "模型 mimo-v2.5 · 服务 MIMOAPI · 上下文 12%"
