...
class TestExtractMedia:
    ...
    def test_media_tag_supports_unquoted_windows_paths(self):
        content = r"已生成图片：MEDIA:E:\AI\hermes\cache\diagram.png"
        media, cleaned = BasePlatformAdapter.extract_media(content)
        assert media == [(r"E:\AI\hermes\cache\diagram.png", False)]
        assert "MEDIA:" not in cleaned
        assert r"E:\AI\hermes\cache\diagram.png" not in cleaned
        assert "已生成图片" in cleaned

    def test_media_tag_supports_windows_paths_with_spaces(self):
        content = r"MEDIA:C:\Users\Administrator\AppData\Local\Temp\my image.webp"
        media, cleaned = BasePlatformAdapter.extract_media(content)
        assert media == [(r"C:\Users\Administrator\AppData\Local\Temp\my image.webp", False)]
        assert cleaned == ""
