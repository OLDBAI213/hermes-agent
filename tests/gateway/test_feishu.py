...
class TestFeishuMessageNormalization(unittest.TestCase):
    ...
    def test_normalize_share_chat_exposes_summary_and_metadata(self):
        ...
        self.assertEqual(normalized.text_content, "共享聊天: Backend Guild\n聊天 ID: oc_chat_shared")
...
class TestFeishuAdapterMessaging(unittest.TestCase):
    ...
    def test_load_settings_reads_outbound_format_and_card_mode(self):
        from gateway.config import PlatformConfig
        from gateway.platforms.feishu import FeishuAdapter

        adapter = FeishuAdapter(PlatformConfig(extra={
            "outbound_format": "card",
            "card_mode": True,
        }))

        self.assertEqual(adapter._outbound_format, "card")
        self.assertTrue(adapter._card_mode)

    @patch.dict(os.environ, {
        "FEISHU_APP_ID": "cli_app",
        "FEISHU_APP_SECRET": "secret_app",
    }, clear=True)
    def test_build_outbound_payload_card_mode_uses_interactive_card(self):
        from gateway.config import PlatformConfig
        from gateway.platforms.feishu import FeishuAdapter
        ...
        self.assertTrue(parsed["config"]["wide_screen_mode"])
        self.assertEqual(parsed["header"]["title"]["content"], "Hermes · 飞书")
        self.assertEqual(parsed["elements"][0]["tag"], "markdown")
        self.assertIn("## 标题", parsed["elements"][0]["content"])

    def test_build_runtime_footer_card_payload(self):
        from gateway.platforms.feishu import _build_runtime_footer_card_payload

        parsed = json.loads(_build_runtime_footer_card_payload("模型 mimo-v2.5 · 服务 MIMOAPI · 上下文 12%"))

        self.assertEqual(parsed["header"]["title"]["content"], "📌 运行状态")
        self.assertEqual(parsed["header"]["template"], "green")
        self.assertEqual(parsed["elements"][0]["tag"], "markdown")
        self.assertIn("上下文 12%", parsed["elements"][0]["content"])

    @patch.dict(os.environ, {
        "FEISHU_APP_ID": "cli_app",
        "FEISHU_APP_SECRET": "secret_app",
    }, clear=True)
    def test_build_outbound_payload_forced_post_uses_post_even_for_plain_text(self):
        from gateway.config import PlatformConfig
        from gateway.platforms.feishu import FeishuAdapter

        adapter = FeishuAdapter(PlatformConfig(extra={"outbound_format": "post"}))
        msg_type, payload = adapter._build_outbound_payload("纯文本")
        parsed = json.loads(payload)
    ...
    def test_markdown_post_polishes_split_structured_list_for_desktop_readability(self):
        from gateway.platforms.feishu import _build_markdown_post_payload

        content = "\n".join(
            [
                "✅ 安全方案",
                "防火墙: 已开启（保护网络不被攻击）",
                "Defender 排除项: 已添加（不扫描你的文件）",
                "排除类型",
                "目录",
                "D:\\ E:\\ F:\\ G:\\（整个盘）",
                "目录",
                "Program Files、AppData、Desktop、Downloads",
                "文件类型",
                ".exe .dll .sys .msi .py .js .ts .go .rs",
                "效果:",
            ]
        )

        parsed = json.loads(_build_markdown_post_payload(content))
        text = "\n".join(row[0]["text"] for row in parsed["zh_cn"]["content"])

        self.assertIn("排除类型:", text)
        self.assertIn("- 目录: D:\\ E:\\ F:\\ G:\\（整个盘）", text)
        self.assertIn("- 目录: Program Files、AppData、Desktop、Downloads", text)
        self.assertIn("- 文件类型: .exe .dll .sys .msi .py .js .ts .go .rs", text)
        self.assertNotIn("\n目录\nD:\\", text)

    def test_markdown_post_polisher_does_not_rewrite_code_blocks(self):
        from gateway.platforms.feishu import _build_markdown_post_payload
...
class TestAdapterBehavior(unittest.TestCase):
    ...
    def test_extract_share_chat_message_as_text_summary(self):
        ...
        self.assertEqual(text, "共享聊天: Platform Ops\n聊天 ID: oc_shared")
        self.assertEqual(msg_type.value, "text")
        self.assertEqual(media_urls, [])
        self.assertEqual(media_types, [])
    ...
    @patch.dict(os.environ, {}, clear=True)
    def test_extract_image_message_explains_download_failure(self):
        from gateway.config import PlatformConfig
        from gateway.platforms.feishu import FeishuAdapter
        ...
        self.assertIn("图片下载失败", text)
        self.assertIn("权限不足", text)
        self.assertEqual(text.count("图片下载失败"), 1)
        self.assertEqual(msg_type.value, "photo")
        self.assertEqual(media_urls, [])
        self.assertEqual(media_types, [])
    ...
    @patch.dict(os.environ, {}, clear=True)
    def test_extract_file_message_explains_download_failure(self):
        from gateway.config import PlatformConfig
        from gateway.platforms.feishu import FeishuAdapter
        ...
        self.assertIn("文件下载失败: report.pdf", text)
        self.assertIn("可能已过期", text)
        self.assertEqual(text.count("文件下载失败"), 1)
        self.assertEqual(msg_type.value, "document")
        self.assertEqual(media_urls, [])
        self.assertEqual(media_types, [])
    ...
    def test_extract_text_from_raw_content_uses_relation_message_fallbacks(self):
        ...
        self.assertEqual(shared, "共享聊天: Platform Ops\n聊天 ID: oc_shared")
        self.assertEqual(attachment, "[附件: report.pdf]")
    ...
    def test_send_animation_degrades_to_document_send(self):
        ...
        self.assertIn("GIF 已转为文件发送", caption)
    ...
    def test_build_post_payload_extracts_title_and_links(self):
        ...
        self.assertEqual(rows[0], [{"tag": "text", "text": "标题", "style": {"bold": True}}])
        self.assertEqual(
            rows[1],
            [
                {"tag": "text", "text": "访问 "},
    ...
    def test_build_post_payload_never_emits_empty_text_elements(self):
        ...
        adapter = FeishuAdapter(PlatformConfig())
        payload = json.loads(adapter._build_post_payload("第一行\n\n第二行"))

        self.assertEqual(payload["zh_cn"]["title"], "")
        for row in payload["zh_cn"]["content"]:
            for element in row:
                if element.get("tag") == "text":
                    self.assertNotEqual(element.get("text"), "")

    @patch.dict(os.environ, {}, clear=True)
    def test_build_post_payload_converts_markdown_to_native_elements(self):
        from gateway.config import PlatformConfig
        from gateway.platforms.feishu import FeishuAdapter
        ...
        self.assertEqual(
            ...
            [
                ...
                {"tag": "text", "text": "、"},
                {"tag": "text", "text": "斜体", "style": {"italic": True}},
                {"tag": "text", "text": " 和 "},
                {"tag": "text", "text": "代码", "style": {"code": True}},
            ],
        )
    ...
    def test_build_post_payload_strips_raw_markdown_markers(self):
        ...
        rows = payload["zh_cn"]["content"]
        flattened = "\n".join("".join(item.get("text", "") for item in row) for row in rows)
        self.assertIn("————", flattened)
        self.assertIn("1. 第一项", flattened)
        self.assertIn("下划线 和 删除线", flattened)
        self.assertNotIn("---", flattened)
        self.assertNotIn("<u>", flattened)
        self.assertNotIn("~~", flattened)
    ...
    def test_send_splits_fenced_code_blocks_into_separate_post_rows(self):
        ...
        rows = payload["zh_cn"]["content"]
        flattened = "\n".join("".join(item.get("text", "") for item in row) for row in rows)
        self.assertIn("确认已入库 ✓", flattened)
        self.assertIn("/root/.hermes/profiles/agent_cto/cron/jobs.json", flattened)
        self.assertIn("解码后的内容：", flattened)
        self.assertIn('{"cron": "list"}', flattened)
        self.assertIn("后续说明仍应保留。", flattened)
        self.assertNotIn("```", flattened)
        self.assertNotIn("**", flattened)
    ...
    def test_build_post_payload_keeps_fence_like_code_lines_inside_code_block(self):
        ...
        self.assertEqual(
            ...
            [
                ...
                [{"tag": "text", "text": "```oops", "style": {"code": True}}],
                [{"tag": "text", "text": "after"}],
            ],
        )
    ...
    def test_build_post_payload_preserves_trailing_spaces_in_code_block(self):
        ...
        self.assertEqual(
            ...
            [
                [{"tag": "text", "text": "before"}],
                [{"tag": "text", "text": "line with two spaces  ", "style": {"code": True}}],
                [{"tag": "text", "text": "after"}],
            ],
        )
    ...
    def test_build_post_payload_splits_multiple_fenced_code_blocks(self):
        ...
        self.assertEqual(
            ...
            [
                [{"tag": "text", "text": "before"}],
                [{"tag": "text", "text": "print(1)", "style": {"code": True}}],
                [{"tag": "text", "text": "middle"}],
                [{"tag": "text", "text": "{}", "style": {"code": True}}],
                [{"tag": "text", "text": "after"}],
            ],
        )
    ...
    def test_send_uses_post_for_advanced_markdown_lines(self):
        ...
        self.assertEqual(
            ...
            [
                [{"tag": "text", "text": "————"}],
                [{"tag": "text", "text": "1. 第一项"}],
                [{"tag": "text", "text": "下划线", "style": {"underline": True}}],
                [{"tag": "text", "text": "删除线", "style": {"strikethrough": True}}],
            ],
        )
...
class TestProcessingReactions(unittest.TestCase):
    ...
    @patch.dict(os.environ, {}, clear=True)
    def test_start_sends_immediate_status_message(self):
        from gateway.config import Platform
        from gateway.session import SessionSource

        adapter, _tracker = self._build_adapter(next_reaction_id="r_typing")
        sent = []
        ...
        event = SimpleNamespace(message_id="om_msg", source=source)
        config = {
            "model": {
                "default": "mimo-v2.5",
                "provider": "xiaomi",
        ...
        with patch("gateway.platforms.feishu.load_config_readonly", return_value=config), \
             patch("gateway.platforms.feishu.get_hermes_home", return_value=Path("Z:/missing")), \
             self._patch_to_thread():
            self._run(adapter.on_processing_start(event))

        self.assertEqual(sent[0]["chat_id"], "oc_chat")
        self.assertEqual(sent[0]["reply_to"], "om_msg")
        self.assertEqual(sent[0]["metadata"], {"thread_id": "topic_1", "reply_to_message_id": "om_msg"})
        self.assertIn("⌛ 已收到，正在思考...", sent[0]["content"])
        self.assertIn("模型:     mimo-v2.5", sent[0]["content"])
        self.assertIn("服务商:   小米 MiMo", sent[0]["content"])
        self.assertIn("上下文:   约 0 / 1M (0%)", sent[0]["content"])
