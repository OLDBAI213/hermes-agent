...
@pytest.mark.asyncio
async def test_handle_model_command_feishu_zh_localizes_unknown_model(tmp_path, monkeypatch):
    hermes_home = tmp_path / ".hermes"
    hermes_home.mkdir()
    (hermes_home / "config.yaml").write_text(
        yaml.safe_dump(
            {
                "display": {"language": "zh", "gateway_locale": "zh"},
    ...
    result = await _make_runner()._handle_model_command(_make_event(platform=Platform.FEISHU))
