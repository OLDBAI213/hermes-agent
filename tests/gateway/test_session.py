...
class TestGatewayAttachmentContext:
    @pytest.fixture()
    def runner(self):
        from gateway.run import GatewayRunner
    ...
    @pytest.mark.asyncio
    async def test_non_document_message_with_pdf_attachment_gets_file_path_note(self, runner, source, tmp_path):
        pdf_path = tmp_path / "feishu_123_report.pdf"
        pdf_path.write_bytes(b"%PDF-1.4\n")
        event = MessageEvent(
            text="[附件: report.pdf]",
            source=source,
            media_urls=[str(pdf_path)],
        ...
        assert "[附件: report.pdf]" in result

    @pytest.mark.asyncio
    async def test_non_document_message_with_text_attachment_injects_content(self, runner, source, tmp_path):
        txt_path = tmp_path / "feishu_123_notes.txt"
        txt_path.write_text("hello from embedded text file", encoding="utf-8")
        event = MessageEvent(
            text="[附件: notes.txt]",
            source=source,
            media_urls=[str(txt_path)],
        ...
        assert "The user sent a text document: 'notes.txt'" in result
        assert "hello from embedded text file" in result
        assert str(txt_path) in result
