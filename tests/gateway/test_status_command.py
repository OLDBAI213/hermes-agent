...
@pytest.mark.asyncio
async def test_status_command_uses_compact_chinese_on_feishu(monkeypatch):
    import gateway.run as gateway_run

    source = _make_source(Platform.FEISHU)
    session_key = build_session_key(source)
    session_entry = SessionEntry(
        session_key=session_key,
        session_id="sess-feishu-compact",
        created_at=datetime.now(),
        updated_at=datetime.now(),
    ...
    runner.adapters[Platform.FEISHU]._pending_messages = {}
    runner._queued_events = {session_key: [MagicMock()]}
    runner._running_agents[session_key] = MagicMock()
    runner._session_db.get_session_title.return_value = "移动会话"
    runner._session_db.get_session.return_value = {
        "input_tokens": 100,
        "output_tokens": 20,
        "cache_read_tokens": 3,
    ...
    result = await runner._handle_message(_make_event("/status", platform=Platform.FEISHU))

    assert "📊 **当前状态**" in result
    assert "💬 会话：移动会话" in result
    assert "⚙️ 运行：是" in result
    assert "📥 队列：1 条待处理" in result
    assert "🧮 Token：130" in result
    assert "🔌 连接：飞书" in result
    assert "🧭 最近：看这张图 [图片/附件]" in result
