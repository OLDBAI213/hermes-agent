...
class FailingFeishuEditProgressAdapter(ProgressCaptureAdapter):
    async def edit_message(self, chat_id, message_id, content) -> SendResult:
        self.edits.append(
            {
                "chat_id": chat_id,
        ...
        return SendResult(
            success=False,
            error="[230001] invalid message content",
            message_id=message_id,
        )
...
class ProgressCommentaryProgressAgent:
    def __init__(self, **kwargs):
        self.tool_progress_callback = kwargs.get("tool_progress_callback")
    ...
    def run_conversation(self, message, conversation_history=None, task_id=None):
        self.tool_progress_callback("tool.started", "terminal", "first command", {})
        time.sleep(0.45)
        self.interim_assistant_callback("interim note", already_streamed=False)
        time.sleep(0.45)
        self.tool_progress_callback("tool.started", "web_search", "second command", {})
        time.sleep(0.55)
        return {
            "final_response": "done",
            "messages": [],
...
class FailedToolProgressAgent:
    ...
    def run_conversation(self, message, conversation_history=None, task_id=None):
        ...
        cb("tool.started", "web_extract", "https://example.com/broken", {})
        time.sleep(0.35)
        cb(
            "tool.completed",
            "web_extract",
            ...
            duration=0.4,
            is_error=True,
            result_preview="Error executing tool 'web_extract': timed out",
        )
        time.sleep(0.1)
        return {
            "final_response": "done",
...
async def test_run_agent_feishu_progress_replies_inside_existing_thread(monkeypatch, tmp_path):
    ...
    assert result["final_response"] == "done"
    assert adapter.sent
    progress_send = next(call for call in adapter.sent if "terminal" in call["content"])
    assert progress_send["reply_to"] == "om_triggering_user_message"
    assert progress_send["metadata"] == {"thread_id": "topic_17585"}
    assert adapter.edits
    assert adapter.edits[0]["message_id"] == "progress-1"


@pytest.mark.asyncio
async def test_feishu_keeps_one_progress_bubble_across_interim_messages(monkeypatch, tmp_path):
    adapter, result = await _run_with_agent(
        monkeypatch,
        tmp_path,
        ProgressCommentaryProgressAgent,
        session_id="sess-feishu-one-progress-bubble",
        platform=Platform.FEISHU,
        config_data={
            "display": {
                "interim_assistant_messages": True,
                "platforms": {"feishu": {"tool_progress": "all"}},
            },
    ...
    assert result["final_response"] == "done"
    progress_sends = [call for call in adapter.sent if "terminal" in call["content"] or "web_search" in call["content"]]
    assert len(progress_sends) == 1
    progress_versions = [call["content"] for call in progress_sends] + [edit["content"] for edit in adapter.edits]
    assert any("terminal" in content and "web_search" in content for content in progress_versions), progress_versions


@pytest.mark.asyncio
async def test_feishu_edit_failure_does_not_send_standalone_tool_lines(monkeypatch, tmp_path):
    adapter, result = await _run_with_agent(
        monkeypatch,
        tmp_path,
        DelayedProgressAgent,
        session_id="sess-feishu-edit-failure-no-fragments",
        platform=Platform.FEISHU,
        config_data={
            "display": {
                "gateway_locale": "zh",
        ...
        adapter_cls=FailingFeishuEditProgressAdapter,
    )

    assert result["final_response"] == "done"
    assert len(adapter.sent) == 1
    assert adapter.sent[0]["content"].startswith("🧰 工具调用记录"), adapter.sent
    assert adapter.edits
    assert not any(
        call["content"] in {'💻 terminal: "second command"', '💻 终端: "second command"'}
        for call in adapter.sent
    )


@pytest.mark.asyncio
async def test_feishu_zh_progress_appends_failed_tool_line(monkeypatch, tmp_path):
    adapter, result = await _run_with_agent(
        monkeypatch,
        tmp_path,
        FailedToolProgressAgent,
        session_id="sess-feishu-tool-failure-line",
        platform=Platform.FEISHU,
        config_data={
            "display": {
                "gateway_locale": "zh",
    ...
    assert result["final_response"] == "done"
    progress_versions = [call["content"] for call in adapter.sent] + [edit["content"] for edit in adapter.edits]
    assert any("📄 网页提取" in content for content in progress_versions), progress_versions
    assert any("⚠️ 网页提取失败: timed out" in content for content in progress_versions), progress_versions
    assert any(content.startswith("🧰 工具调用记录（1次）\n1. ") for content in progress_versions), progress_versions
    assert any(content.startswith("🧰 工具调用记录（2次）\n1. ") for content in progress_versions), progress_versions
    assert any("\n2. ⚠️ 网页提取失败: timed out" in content for content in progress_versions), progress_versions
    assert not any("web_extract" in content for content in progress_versions), progress_versions
