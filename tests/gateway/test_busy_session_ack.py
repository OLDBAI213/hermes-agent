...
def _make_event(text="hello", chat_id="123", platform_val="telegram", media_urls=None, media_types=None):
...
class TestBusySessionAck:
    ...
    @pytest.mark.asyncio
    async def test_steer_mode_queues_media_followup_so_images_survive(self):
        """Media follow-ups must not go through agent.steer(), which only carries text."""
        runner, sentinel = _make_runner()
        runner._busy_input_mode = "steer"
        adapter = _make_adapter()
        ...
        event = _make_event(
            text="[图片]\n看这个",
            media_urls=["E:/AI/hermes/image_cache/img_test.jpg"],
        ...
        assert queued.media_urls == ["E:/AI/hermes/image_cache/img_test.jpg"]
        assert queued.media_types == ["image/jpeg"]
        content = adapter._send_with_retry.call_args.kwargs.get("content", "")
        assert "Queued for the next turn" in content
        assert "image/media" in content
