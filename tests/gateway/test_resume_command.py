...
class TestHandleResumeCommand:
    ...
    @pytest.mark.asyncio
    async def test_feishu_list_named_sessions_is_compact_chinese(self, tmp_path):
        """Feishu /resume without args uses a compact mobile-friendly list."""
        from hermes_state import SessionDB
        db = SessionDB(db_path=tmp_path / "state.db")
        db.create_session("sess_001", "feishu")
        db.create_session("sess_002", "feishu")
        db.set_session_title("sess_001", "Research")
        db.set_session_title("sess_002", "Coding")

        event = _make_event(text="/resume", platform=Platform.FEISHU)
        runner = _make_runner(session_db=db, event=event)
        result = await runner._handle_resume_command(event)
        assert "可恢复会话" in result
        assert "Research" in result
        assert "Coding" in result
        assert "发送 /resume 标题 恢复" in result
