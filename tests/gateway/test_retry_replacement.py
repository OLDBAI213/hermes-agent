...
async def test_gateway_retry_rejects_media_history_without_rewriting(tmp_path):
    ...
    assert "image" in result.lower() or "图片" in result or "檔案" in result
...
@pytest.mark.asyncio
async def test_gateway_retry_replays_text_blocks(tmp_path):
    config = MagicMock()
    config.sessions_dir = tmp_path
    config.max_context_messages = 20
    gw = GatewayRunner.__new__(GatewayRunner)
    ...
    gw.session_store.rewrite_transcript.assert_called_once()
