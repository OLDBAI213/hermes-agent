...
class TestChromeDebugLaunch:
    ...
    def test_find_ready_browser_debug_url_checks_ipv6_when_ipv4_is_not_cdp(self):
        requested = []

        def fake_urlopen(url, timeout):
            requested.append(url)
            if url == "http://[::1]:9222/json/version":
                return _FakeResponse()
            raise OSError("not cdp")

        with patch("urllib.request.urlopen", side_effect=fake_urlopen):
            assert find_ready_browser_debug_url(9222, timeout=0.1) == "http://[::1]:9222"
