...
class TestSendMessageTool:
    ...
    def test_windows_media_path_is_sent_as_attachment_not_text(self):
        config, telegram_cfg = _make_config()
        image_path = r"E:\AI\hermes\cache\result.png"
...
class TestSendFeishuMediaDelivery:
    def test_missing_media_returns_chinese_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(feishu_mod, "FeishuAdapter", FakeFeishuAdapter)
        ...
        assert "error" in result
        assert "媒体文件不存在" in result["error"]
        assert "Media file not found" not in result["error"]

    def test_empty_media_result_returns_chinese_error(self, monkeypatch):
        from gateway.platforms import feishu as feishu_mod
        ...
        result = asyncio.run(_send_feishu(SimpleNamespace(extra={}), "oc_chat", "", media_files=[]))

        assert "error" in result
        assert "没有可发送的文字或媒体内容" in result["error"]
        assert "No deliverable text or media" not in result["error"]

    def test_text_send_failure_returns_chinese_error(self, monkeypatch):
        from gateway.platforms import feishu as feishu_mod
        ...
        class FakeFeishuAdapter:
            ...
            async def send(self, chat_id, message, metadata=None):
                return SimpleNamespace(success=False, error="boom", message_id=None)

        monkeypatch.setattr(feishu_mod, "FEISHU_AVAILABLE", True)
        monkeypatch.setattr(feishu_mod, "FeishuAdapter", FakeFeishuAdapter)

        result = asyncio.run(_send_feishu(SimpleNamespace(extra={}), "oc_chat", "hello", media_files=[]))

        assert "error" in result
        assert "飞书发送失败" in result["error"]
        assert "Feishu send failed" not in result["error"]
