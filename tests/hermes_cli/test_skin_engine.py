...
class TestUserSkins:
    def test_load_user_skin_from_yaml(self, tmp_path, monkeypatch):
        ...
        skin_data = {
            ...
            "visuals": {"character_frames": ["FRAME_A", "FRAME_B"]},
            "tool_prefix": "▸",
        }
        import yaml
        ...
        assert skin.visuals["character_frames"] == ["FRAME_A", "FRAME_B"]
        assert skin.tool_prefix == "▸"
        # Should inherit defaults for unspecified colors
        assert skin.get_color("banner_border") == "#CD7F32"  # from default

    def test_load_user_skin_preserves_banner_assets(self, monkeypatch):
        from hermes_cli.skin_engine import load_skin

        skins_dir = Path(__file__).parents[1] / "fixtures" / "skins"
        logo = "[#31C476]HE[/][#D9DBD6]RMES[/]\n[#00453E]+-----+[/]"
        hero = "[#31C476]表x[/]\n[#D9DBD6]+---+[/]"
        monkeypatch.setattr("hermes_cli.skin_engine._skins_dir", lambda: skins_dir)

        skin = load_skin("asset-pack")

        assert skin.banner_logo == logo
        assert skin.banner_hero == hero
