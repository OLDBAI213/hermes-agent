...
class HermesCLI:
    ...
    def _handle_browser_command(self, cmd: str):
        ...
        if sub.startswith("connect"):
            ...
            custom_cdp = len(connect_parts) > 2
            cdp_url = connect_parts[2].strip() if custom_cdp else _DEFAULT_CDP
            ...
            ready_default_cdp = None if custom_cdp else find_ready_browser_debug_url(_port, timeout=1.0)
            if ready_default_cdp:
                cdp_url = ready_default_cdp
                parsed_cdp = urlparse(cdp_url)
                _port = parsed_cdp.port or DEFAULT_BROWSER_CDP_PORT

            _already_open = bool(ready_default_cdp) or is_browser_debug_ready(cdp_url, timeout=1.0)
            ...
            if _already_open:
                ...
                if _launched:
                    ...
                    for _wait in range(10):
                        ready_default_cdp = find_ready_browser_debug_url(_port, timeout=1.0)
                        if ready_default_cdp:
                            cdp_url = ready_default_cdp
                            _already_open = True
                            break
