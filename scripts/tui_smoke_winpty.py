...
def main() -> int:
    ...
    parser.add_argument(
        "--command",
        help="Optional command text to type after startup markers, then submit with a real ConPTY Enter key.",
    )
    parser.add_argument(
        "--slash-command",
        help="Slash command to submit; appends a trailing space so the completion menu cannot hold Enter.",
    )
    parser.add_argument(
        "--input-delay",
        type=float,
        default=0.15,
        help="Seconds to wait between typing --command and sending Enter.",
    )
    parser.add_argument(
        "--before-input-delay",
        type=float,
        default=0.0,
        help="Seconds to wait after startup markers before typing --input/--command.",
    )
    parser.add_argument(
        "--submit-enters",
        type=int,
        default=1,
        help="Number of Enter keypresses to send after --command.",
    )
    parser.add_argument(
        "--after-input-marker",
        action="append",
    ...
    selected_inputs = [bool(args.input), bool(args.command), bool(args.slash_command)]
    if sum(selected_inputs) > 1:
        return fail("--input, --command, and --slash-command are mutually exclusive", code=2)
    ...
    input_sent = False
    pending_input_at: float | None = None
    pending_enter_at: float | None = None
    pending_enters = 0
    startup_seen = False
    ...
    def print_pass() -> None:
        ...
        if args.command or args.slash_command:
    ...
    try:
        ...
        def send_startup_input() -> None:
            nonlocal input_sent, pending_enter_at, pending_enters

            if input_sent or proc is None:
                return

            if args.command or args.slash_command:
                command = args.command if args.command is not None else args.slash_command.rstrip() + " "
                proc.write(command)
                pending_enters = max(1, args.submit_enters)
                pending_enter_at = time.monotonic() + max(0, args.input_delay)
            else:
                proc.write(args.input)
            input_sent = True

        def maybe_send_delayed_input() -> None:
            nonlocal pending_input_at

            if pending_input_at is None or time.monotonic() < pending_input_at:
                return

            pending_input_at = None
            send_startup_input()

        def maybe_submit_command_enter() -> None:
            nonlocal pending_enter_at, pending_enters

            if pending_enter_at is None or pending_enters <= 0 or proc is None:
                return

            if time.monotonic() < pending_enter_at:
                return

            proc.write("\r")
            pending_enters -= 1
            pending_enter_at = time.monotonic() + max(0.01, args.input_delay) if pending_enters > 0 else None

        while time.monotonic() < deadline:
            maybe_send_delayed_input()
            maybe_submit_command_enter()
            ...
            if not startup_seen and contains_all(sanitized, markers):
                ...
                if not args.input and not args.command and not args.slash_command:
                    if maybe_resize():
                        continue
                ...
                if args.before_input_delay > 0:
                    pending_input_at = time.monotonic() + args.before_input_delay
                else:
                    send_startup_input()
