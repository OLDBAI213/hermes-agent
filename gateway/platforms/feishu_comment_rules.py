...
def _print_status() -> None:
    ...
    print(f"  允许来源:  {sorted(cfg.allow_from) if cfg.allow_from else '[]'}")
    print()
    if cfg.documents:
        ...
        for key, rule in sorted(cfg.documents.items()):
            ...
            print(f"  [{key}] {', '.join(parts) if parts else '（空，继承全部）'}")
    else:
        print("文档规则: 无")
    print()
...
def _do_check(doc_key: str, user_open_id: str) -> None:
    ...
    print(f"  允许来源:   {sorted(rule.allow_from) if rule.allow_from else '[]'}")
    print(f"  匹配来源:   {rule.match_source}")
...
def _main() -> int:
    ...
    usage = (
        ...
        "  pairing add <user_open_id>           添加已配对用户\n"
        "  pairing remove <user_open_id>        移除已配对用户\n"
        "  pairing list                         列出已配对用户\n"
        "\n"
        f"规则配置文件: {RULES_FILE}\n"
        "  可直接编辑这个 JSON 文件来配置策略和文档规则。\n"
        "  修改会在下一次评论事件生效，无需重启。\n"
    )
