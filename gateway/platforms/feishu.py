...
def _web_response(*, status: int = 200, text: str = "") -> Any:
    if web is not None:
...
def _text_post_element(text: str, **style: bool) -> Dict[str, Any]:
    ...
    enabled_style = {key: True for key, value in style.items() if value}
    if enabled_style:
...
def _markdown_line_to_post_row(line: str) -> List[Dict[str, Any]]:
    stripped = line.strip()
    if not stripped:
    ...
    if heading:
        return [_text_post_element(heading.group(1).strip(), bold=True)]
...
def _build_markdown_post_rows(content: str) -> List[List[Dict[str, Any]]]:
    """Build Feishu post rows with native formatting instead of raw Markdown.

    Real Feishu post messages do not reliably render GitHub-style Markdown
    markers inside ``md`` rows. Convert the common syntax Hermes emits
    (headings, bold, inline code, links, fences) to native post elements so the
    client does not show raw ``**``/`````/``##`` markers.
    """
    if not content:
        return [[_text_post_element("")]]
    ...
    for raw_line in content.splitlines():
        ...
        rows.append(_markdown_line_to_post_row(raw_line))

    if in_code_block:
        _flush_code()
    return rows or [[_text_post_element(content)]]
...
def _render_post_element(
    ...
    if tag == "at":
        ...
        if ref is not None:
            display_name = ref.name or ref.open_id or "用户"
        else:
            display_name = str(element.get("user_name", "")).strip() or "用户"
...
def _normalize_feishu_text(
    ...
    def _sub(match: "re.Match[str]") -> str:
        ...
        if ref is None:
            return " "
        name = ref.name or ref.open_id or "用户"
        return f"@{name}"
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        return FeishuAdapterSettings(
            ...
            outbound_audit=_to_boolean(
                extra.get("outbound_audit", os.getenv("HERMES_FEISHU_OUTBOUND_AUDIT", "false"))
            ),
            outbound_audit_full_payload=_to_boolean(
                extra.get(
                    "outbound_audit_full_payload",
                    os.getenv("HERMES_FEISHU_OUTBOUND_AUDIT_FULL_PAYLOAD", "false"),
                )
            ),
        )
    ...
    def _apply_settings(self, settings: FeishuAdapterSettings) -> None:
        ...
        self._outbound_audit = settings.outbound_audit
    ...
    def _audit_outbound_message(
        ...
        *,
        ...
        if not getattr(self, "_outbound_audit", False):
            return
        try:
            ...
            entry: Dict[str, Any] = {
                ...
                "payload_preview": payload_text[:4000],
                "metadata_keys": sorted((metadata or {}).keys()),
            }
            if getattr(self, "_outbound_audit_full_payload", False):
                entry["payload"] = payload_text
            if result is not None:
                entry["result_success"] = result.success
                entry["result_message_id"] = result.message_id
                entry["result_error"] = result.error
            if error:
                entry["error"] = error
            with (log_dir / "feishu-outbound.ndjson").open("a", encoding="utf-8") as handle:
                handle.write(json.dumps(entry, ensure_ascii=False, default=str) + "\n")
        except Exception as exc:
            logger.debug("[Feishu] Failed to write outbound audit log: %s", exc, exc_info=True)
        ...
        try:
            for chunk in chunks:
                ...
                try:
                    ...
                    fallback_payload = json.dumps({"text": _strip_markdown_to_plain_text(chunk)}, ensure_ascii=False)
                    self._audit_outbound_message(
                        stage="send.fallback_text",
                        chat_id=chat_id,
                        msg_type="text",
                ...
                if (
                    ...
                    fallback_payload = json.dumps({"text": _strip_markdown_to_plain_text(chunk)}, ensure_ascii=False)
                    self._audit_outbound_message(
                        stage="send.fallback_text_response",
                        chat_id=chat_id,
                        msg_type="text",
                        payload=fallback_payload,
                        reply_to=reply_to,
                        metadata=metadata,
                        error=str(getattr(response, "msg", "") or ""),
                    )
                    response = await self._feishu_send_with_retry(
                        chat_id=chat_id,
                        msg_type="text",
    ...
    async def send_runtime_footer(
        ...
        *,
        ...
        """Send runtime metadata as a compact Feishu status card."""
        if not self._client:
            return SendResult(success=False, error="未连接")

        try:
            payload = _build_runtime_footer_card_payload(content)
            self._audit_outbound_message(
                stage="runtime_footer.build",
                chat_id=chat_id,
            ...
            return self._finalize_send_result(response, "运行状态发送失败")
        except Exception as exc:
            logger.error("[Feishu] Runtime footer card send error: %s", exc, exc_info=True)
            return SendResult(success=False, error=str(exc))
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            result = self._finalize_send_result(response, "命令确认卡片发送失败")
            if result.success:
    ...
    async def send_update_prompt(
        ...
        try:
            ...
            result = self._finalize_send_result(response, "更新确认卡片发送失败")
            if result.success:
    ...
    @staticmethod
    def _format_status_token_count(tokens: int) -> str:
        try:
    ...
    def _processing_start_status_text(self, event: MessageEvent) -> Optional[str]:
        ...
        try:
            ...
            cfg = {}
        if not bool(resolve_display_setting(cfg, "feishu", "processing_start_message", True)):
            return None

        model = str(cfg_get(cfg, "model", "default", default="") or "").strip()
        provider = str(cfg_get(cfg, "model", "provider", default="") or "").strip()
        try:
            context_length = int(cfg_get(cfg, "model", "context_length", default=0) or 0)
        except Exception:
            context_length = 0
        ...
        try:
            ...
            if session_key and sessions_file.exists():
                data = json.loads(sessions_file.read_text(encoding="utf-8"))
                entry = (data or {}).get(session_key) or {}
                context_tokens = int(entry.get("last_prompt_tokens") or 0)
        except Exception:
            context_tokens = 0

        provider_label = get_auth_provider_display_name(provider) if provider else ""
        ...
        if context_length > 0:
            ...
            context_line = (
                f"上下文:   约 {self._format_status_token_count(context_tokens)} / "
                f"{self._format_status_token_count(context_length)} ({pct}%)"
            )
        ...
        if provider_label:
            lines.append(f"服务商:   {provider_label}")
        if context_line:
            lines.append(context_line)
        return "\n".join(lines)

    async def _send_processing_start_status(self, event: MessageEvent) -> None:
        source = getattr(event, "source", None)
        ...
        if not chat_id:
            return
        try:
            text = self._processing_start_status_text(event)
            if not text:
                return
            metadata = None
            ...
            await self.send(
                chat_id=chat_id,
                content=text,
                reply_to=message_id if thread_id and message_id else None,
                metadata=metadata,
            )
        except Exception:
            logger.debug("[Feishu] Failed to send processing start status", exc_info=True)

    async def on_processing_start(self, event: MessageEvent) -> None:
        await self._send_processing_start_status(event)
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if not self._check_webhook_rate_limit(rate_key):
            ...
            return _web_response(status=429, text="请求过于频繁")
        ...
        if content_type and content_type != "application/json":
            ...
            return _web_response(status=415, text="不支持的媒体类型")
        ...
        if content_length is not None and content_length > _FEISHU_WEBHOOK_MAX_BODY_BYTES:
            ...
            return _web_response(status=413, text="请求体过大")
        ...
        try:
            ...
            return _web_response(status=408, text="请求超时")
        except Exception:
            self._record_webhook_anomaly(remote_ip, "400")
            return _web_json_response({"code": 400, "msg": "请求体读取失败"}, status=400)
        ...
        try:
            ...
            self._record_webhook_anomaly(remote_ip, "400")
            return _web_json_response({"code": 400, "msg": "无效 JSON"}, status=400)
        ...
        if self._verification_token:
            ...
            if not incoming_token or not hmac.compare_digest(incoming_token, self._verification_token):
                ...
                return _web_response(status=401, text="verification token 无效")
        ...
        if self._encrypt_key and not self._is_webhook_signature_valid(request.headers, body_bytes):
            ...
            return _web_response(status=401, text="签名无效")
        ...
        if payload.get("encrypt"):
            ...
            return _web_json_response({"code": 400, "msg": "暂不支持加密 webhook 请求体"}, status=400)
    ...
    async def _connect_websocket(self) -> None:
        if not FEISHU_WEBSOCKET_AVAILABLE:
            raise RuntimeError("未安装 websockets，websocket 模式不可用")
    ...
    async def _connect_webhook(self) -> None:
        if not FEISHU_WEBHOOK_AVAILABLE:
            raise RuntimeError("未安装 aiohttp，webhook 模式不可用")
        domain = FEISHU_DOMAIN if self._domain_name != "lark" else LARK_DOMAIN
        self._client = self._build_lark_client(domain)
        self._event_handler = self._build_event_handler()
        if self._event_handler is None:
...
def _qr_register_inner(
    *,
    ...
    if _render_qr(qr_url):
        ...
        print(f"  在手机 Feishu / Lark 中打开这个 URL:\n\n  {qr_url}\n")
        print("  提示: 安装 qrcode 后，下次可直接在这里显示可扫描二维码")
    print()
