...
def _send_lifecycle_notice(message: str) -> None:
    """Best-effort Feishu/home-channel notice for Windows task lifecycle."""
    try:
...
def stop(notice_message: str | None = "⚠️ 网关正在关闭，飞书暂时不可用。") -> None:
    ...
    stopped_any = False
    if notice_message and _gateway_pids():
        _send_lifecycle_notice(notice_message)
    if is_task_registered():
...
def restart() -> None:
    ...
    _assert_windows()
    stop("♻️ 网关正在重启，飞书会短暂不可用。")
    # Give Windows a moment to release the listening port.
    time.sleep(1.0)
    start()
    _send_lifecycle_notice("♻️ 网关已上线，Hermes 可以继续使用。")
